(function() {
  ...
  function createButtons() {
      ...
      function showPosition(position) {
          ...
          // Check if React LocationSelector integration is available
          if (typeof window.setLocationData === 'function') {
              // Provide raw coordinates to React component for reverse geocoding
              window.setLocationData({
                  lat: lat,
                  lng: lon
              });
          } else {
              // Fallback: Show coordinates directly if React integration not available
      ...
      function showError(error) {
          ...
          switch(error.code) {
              case error.PERMISSION_DENIED:
                  errorMessage = 'Location access denied. Please enable location permissions in your app/browser settings.';
                  break;
          ...
          // Check if React LocationSelector integration is available for error handling
      ...
      button1.addEventListener('click', async function(e) {
          try {
              ...
              if (typeof window.setLocationError === 'function') {
                  window.setLocationError(`Error in fetch location button: ${error.message}`);
              } else {
